...
Partial Class _Default
    ...
    Private Sub DataList1_ItemCommand(source As Object, e As DataListCommandEventArgs) Handles DataList1.ItemCommand
        ...
        If e.CommandName = "chofer1" Then
            ...
            orden = 0
            SqlDataSource2.SelectCommand = "select max(orden) from tt_lista where chofer=0"
            SqlDataSource2.DataSourceMode = SqlDataSourceMode.DataReader
            data = SqlDataSource2.Select(DataSourceSelectArguments.Empty)
            If data.Read Then
                orden = data.GetValue(0)
            End If
            data.Close()


            SqlDataSource3.InsertCommand = "insert into tt_lista([id_cliente],[orden],[activo],[fecha],recoleccion,chofer,mes,semana) values ('" & id & "','" & orden + 1 & "', '" & 1 & "', '" & fecha.ToString("yyyy-MM-dd") & "', '" & 0 & "', '" & 0 & "', '" & mes & "','" & semana & "')"
            SqlDataSource3.Insert()
            End If
            If e.CommandName = "chofer2" Then
            Dim fecha As Date
            fecha = Date.Now
            Dim mes As String
            mes = Date.Now.Month
            ...
            Dim orden As Integer
            orden = 0
            SqlDataSource2.SelectCommand = "select max(orden) from tt_lista where chofer=1"
            SqlDataSource2.DataSourceMode = SqlDataSourceMode.DataReader
            data = SqlDataSource2.Select(DataSourceSelectArguments.Empty)
            If data.Read Then
                orden = data.GetValue(0)
            End If
            data.Close()
            SqlDataSource3.InsertCommand = "insert into tt_lista([id_cliente],[orden],[activo],[fecha],recoleccion,chofer,mes,semana) values ('" & id & "','" & orden + 1 & "', '" & 1 & "', '" & fecha.ToString("yyyy-MM-dd") & "', '" & 0 & "', '" & 1 & "', '" & mes & "','" & semana & "')"
            SqlDataSource3.Insert()
        End If
    End Sub
    ...
    Protected Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        ...
        SqlDataSource2.Insert()
        Response.Redirect("alta_cliente - Copia.aspx")
